# 新幹線 領収書ダウンロードツール（スマートEX / えきねっと）

指定期間の新幹線の領収書を**まとめて保存**するツールです。Playwright でブラウザを操作し、
各サービスにログイン → 対象期間の領収書を1件ずつ取得して **デスクトップ**に保存します。
**対応サービス**:

| サービス | 事業者 | 取得方法 |
|---|---|---|
| スマートEX | JR東海 | 明細で宛名入力→「印刷」で開く正式領収書を PDF 化 |
| えきねっと | JR東日本 | 「ご利用票兼領収書を発行する」→宛名入力→PDF を直接ダウンロード |

> **エクスプレス予約（JR東海）** も利用できます。予約システムがスマートEXと共通（入口URLのみ相違）の
> ため選択メニューには出していませんが、`--service expy` で同じ流れで取得できます。

**Web UI（フォーム）** と **CLI** の2通り、**From年月〜To年月の期間指定**に対応。Windows / macOS / Linux 対応。
...
> エクスプレス予約（JR東海）は予約システムがスマートEXと共通のためメニューには出しませんが、
> `--service expy` で引き続き利用できます。
...
> **失敗時は `--debug` を付けていなくても**、止まった画面を `output/debug_*.html`
> （フレームがある場合は `_frameN.html` も）に**自動保存**します。さらに**1件も取得できずに
> 終わった場合は、対話実行（CLI）ならブラウザ／Chrome を開いたまま一時停止**し、画面を確認して
> から Enter で閉じられます。まずはこの自動保存HTMLと画面を確認してください。
...
アーキテクチャ・サービス別フロー・セレクタ調整・拡張方法は [DESIGN.md](DESIGN.md) を参照してください。

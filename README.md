...
- **えきねっと（JR東日本）** (`--service eki-net`): ログイン → 規約同意（自動チェック→次へ）→
  ...
  各「ご利用兼領収書を発行する」をクリックして**領収書ファイルをダウンロード**（同名は連番で一意化）。
  - ログイン（ID/PW・追加認証）は手入力。以降は自動で進みます。**宛名入力は使いません**
    （えきねっと側で扱わないため）。
  - セレクタは `providers/ekinet.py` の `SEL` に集約。うまく動かない場合は `--debug` を付けて
    `output/ek_*.html`（各ステップのHTML）を確認し、`SEL` を調整してください。
